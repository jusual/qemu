...
static uint64_t uart_read(void *opaque, hwaddr addr, unsigned int size)
{
    ...
    trace_nrf51_uart_read(addr, size);
    return r;
}
...
static void uart_receive(void *opaque, const uint8_t *buf, int size)
{
   ...
   if (size == 0 || s->rx_fifo_len >= UART_FIFO_LENGTH) {
    ...
    for (i = 0; i < size; i++) {
        ...
        s->rx_fifo[pos] = buf[i];
        s->rx_fifo_len++;
        qemu_log_mask(LOG_TRACE, "uart_receive: pos %u num %u len %u\n", pos, buf[i], s->rx_fifo_len);
    }
...
static int uart_can_receive(void *opaque)
{
    ...
    qemu_log_mask(LOG_TRACE, "uart_can_receive: %d\n", UART_FIFO_LENGTH - s->rx_fifo_len);
